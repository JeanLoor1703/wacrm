...
export function LossReasons() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await db
      ...
      .order('name');
    if (error) toast.error('No se pudo cargar el catálogo de motivos.');
    ...
    const result = reason
      ...
      : await db
          ...
          .insert({
            ...
            name: name.trim(),
            requires_detail: detail,
          })
          .select('id')
          .single();
    if (result.error) toast.error('No se pudo guardar el motivo.');
    else {
      setName('');
      setDetail(false);
      await load();
  ...
  return (
    <section className="border-border/50 space-y-3 border-t pt-4">
      <h3 className="text-sm font-semibold">Motivos de no concretado</h3>
      <p className="text-muted-foreground text-xs">
        Desactivar no elimina el motivo de las obras anteriores.
      </p>
      ...
      {canEdit && (
        <>
          <Label htmlFor="loss-new-name">Nuevo motivo</Label>
          <Input
            id="loss-new-name"
            value={name}
