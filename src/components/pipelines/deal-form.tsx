...
interface Draft {
  ...
  concrete_strength_other: string;
...
export function DealForm({
  ...
  useEffect(() => {
    ...
    setDraft({
      ...
      stage_id:
        requestedStageId || deal?.stage_id || defaultStageId || firstStageId,
      assigned_to: deal?.assigned_to ?? '',
      expected_close_date: deal?.expected_close_date ?? '',
      notes: deal?.notes ?? '',
      work_type: deal?.work_type ?? '',
      work_location: deal?.work_location ?? '',
      concrete_strength: deal?.concrete_strength ?? 'unknown',
      concrete_strength_other: deal?.concrete_strength_other ?? '',
      estimated_volume_m3:
        deal?.estimated_volume_m3 == null
          ? ''
          : String(deal.estimated_volume_m3),
      scheduled_date: deal?.scheduled_date ?? '',
      needs_pump: deal?.needs_pump ?? 'unknown',
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      if (c.error || p.error || r.error)
        setError(
          'No se pudieron cargar los clientes, asesores o motivos. Cierra y vuelve a abrir la obra.'
        );
    });
    return () => {
      cancelled = true;
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      value: draft.value.trim() ? Number(draft.value) : 0,
      ...
      estimated_volume_m3: draft.estimated_volume_m3.trim()
    ...
    if (validation) {
      setError(validation);
      setTab(
        !payload.title || !payload.contact_id
          ? 'commercial'
          : validation.includes('volumen') || validation.includes('resistencia')
            ? 'concrete'
            : 'sale'
      );
      return;
    ...
    try {
      ...
      if (!user || !accountId)
        throw new Error('Necesitas una sesión activa para guardar.');
      const result = deal
        ? await db
            .from('deals')
            .update(payload)
      ...
      <SheetContent side="right" className="bg-popover w-full gap-0 p-0 sm:max-w-2xl">
        <SheetHeader className="border-border/50 shrink-0 border-b p-4 pr-12">
          <SheetTitle>
            {commercial
              ? deal
                ? 'Editar obra / oportunidad'
                : 'Nueva obra / oportunidad'
              : deal
                ? t('editDeal')
                : t('newDeal')}
          </SheetTitle>
          <SheetDescription>
            {commercial
              ? 'Registra lo confirmado por el cliente. Puedes completar los datos después.'
              : 'Gestiona la oportunidad comercial.'}
          </SheetDescription>
        </SheetHeader>
        {draft && (
          <>
            ...
            >
              <TabsList
                className="m-4 grid h-11 w-auto shrink-0 grid-cols-3"
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4">
                ...
                <TabsContent value="commercial" className="space-y-4">
                  ...
                  {draft.contact_id && hasConversation && (
                    <Link
                      href="/inbox"
                ...
                <TabsContent value="concrete" className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field id="work-strength" label="Resistencia">
                      <select
                        id="work-strength"
                        className={selectClass}
                        value={draft.concrete_strength}
                        onChange={(e) =>
                          change(
                            'concrete_strength',
                            e.target.value as ConcreteStrength
                          )
                        }
                      >
                        {STRENGTHS.map((s) => (
                    ...
                    {textField(
                      'scheduled_date',
                      'Fundición / entrega prevista',
                      'date'
                    )}
                    <Field id="work-pump" label="Necesita bomba">
                      <select
                        id="work-pump"
                        className={selectClass}
                        value={draft.needs_pump}
                        onChange={(e) =>
                          change('needs_pump', e.target.value as UnknownBoolean)
                        }
                      >
                        {TRI_STATES.map((s) => (
                          <option key={s.value} value={s.value}>
                    ...
                    <Field id="work-mixer" label="Acceso para mixer">
                      <select
                        id="work-mixer"
                        className={selectClass}
                        value={draft.mixer_access}
                        onChange={(e) =>
                          change(
                            'mixer_access',
                            e.target.value as UnknownBoolean
                          )
                        }
                      >
                        {TRI_STATES.map((s) => (
                  ...
                  <p className="text-muted-foreground text-xs">
                    Registra la resistencia indicada. El sistema no determina
                    requisitos estructurales ni precios.
                  </p>
                </TabsContent>
                <TabsContent value="sale" className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field id="work-stage" label="Etapa *">
                      <select
                        id="work-stage"
                        className={selectClass}
                        value={draft.stage_id}
                        onChange={(e) => change('stage_id', e.target.value)}
                      >
                        {stages.map((s) => (
                    ...
                    <Field id="work-assignee" label="Asesor">
                      <select
                        id="work-assignee"
                        className={selectClass}
                    ...
                    <Field id="work-currency" label="Moneda">
                      <select
                        id="work-currency"
                        className={selectClass}
                  ...
                  {losing && (
                    <div className="border-border space-y-4 rounded-lg border p-4">
                      <p className="text-sm font-semibold">
                        No concretado · motivo obligatorio
                      </p>
                      <Field id="work-reason" label="Motivo de no concretado *">
                        <select
                          id="work-reason"
                          className={selectClass}
                          value={draft.loss_reason_id}
                          onChange={(e) =>
                            change('loss_reason_id', e.target.value)
                          }
                        >
                          ...
                          {reasons
                            .filter(
                              (r) =>
                                r.is_active || r.id === deal?.loss_reason_id
                            )
                            .map((r) => (
                      ...
                      <Field
                        id="work-reason-detail"
                        label={
                          reasons.find((r) => r.id === draft.loss_reason_id)
                            ?.requires_detail
                      ...
                      >
                        <Textarea
                          id="work-reason-detail"
                          value={draft.loss_reason_detail}
                          onChange={(e) =>
                            change('loss_reason_detail', e.target.value)
                          }
                          className="bg-muted"
                        />
                      </Field>
                      <p className="text-muted-foreground text-xs">
                        La etapa anterior se conserva si cancelas sin guardar.
                      </p>
                    </div>
                  )}
                  <Field id="work-notes" label="Notas">
                    <Textarea
                      id="work-notes"
                      value={draft.notes}
                  ...
                  {deal && (
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        {commercial ? 'No concretado' : t('markAsLost')}
                      </Button>
                      {deal.status !== 'open' && (
                        <Button
                          type="button"
                      ...
                      {commercial && (
                        <p className="text-muted-foreground w-full text-xs">
                          Confirma el cambio con Guardar cambios.
                        </p>
                      )}
                    </div>
                  )}
