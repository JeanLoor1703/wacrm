...
import { CrmSegmentsPanel } from '@/components/broadcasts/crm-segments-panel';
...
export default function BroadcastsPage() {
  ...
  const fetchBroadcasts = useCallback(async () => {
    try {
      ...
      const [broadcastResult, configResult] = await Promise.all([
        supabase
          .from('broadcasts')
          .select('*')
      ...
      setWhatsAppConnected(Boolean(configResult.data));
    } catch (err) {
      ...
      {!whatsAppConnected && (
        ...
        >
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Puedes analizar segmentos y guardar borradores. El envío permanece
            bloqueado.
          </p>
        </div>
      )}
