...
it('loads official aggregates without a client row limit or date range', async () => {
  ...
  expect(
    await loadCommercialMetrics({ rpc } as unknown as SupabaseClient)
  ).toEqual(data);
...
it('scopes a board summary to its pipeline', async () => {
  ...
  expect(rpc).toHaveBeenCalledWith('creacom_metrics', {
    p_pipeline_id: 'demo',
  });
});
it('does not invent zero figures on query failure', async () => {
  ...
  await expect(
    loadCommercialMetrics({ rpc } as unknown as SupabaseClient)
  ).rejects.toThrow(/métricas/);
});
